...
$token = $_COOKIE['token'] ?? $data['token'] ?? null;

if (!$token) {
    echo json_encode(['success' => false, 'error' => 'token no encontrada o expirada']);
    exit();
}
...
if (openssl_private_decrypt($encryptedData, $decrypted_data, $private_key)) {
    ...
    if (strtotime($tokenExpire) < strtotime($timeActual)) {
        ...
        echo json_encode(['success' => false, 'error' => 'El token ya expiró.']);
        exit();
    }
...
$stmt = $conex->prepare("UPDATE usuario SET contrasena_hash = ? WHERE usuario_id = ?");
$stmt->bind_param("si", $contrasena_hash,$usuario_id);
if($stmt->execute()){
    echo json_encode(['success'=>true,'resultado'=>'contraseña actualizada.']);
}
$stmt->close();
$conex->close();
} catch (Exception $e) {
